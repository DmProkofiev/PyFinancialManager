...
class FinanceService():
    ...
    def calculate_balance(self) -> float:
        this_income = sum(i.amount for i in self.get_income())
        this_expense = sum(i.amount for i in self.get_expense())
        return this_income - this_expense
...
# CREATE
    ...
    def add_expense(self, amount: float, expense_type: ExpenseType, description: str, date: Optional[datetime]):
        item = Expense(amount = amount, type = expense_type, description = description, date=date)
        self._repo.add_expense(item)

    def add_income(self, amount: float, income_type: IncomeType, description: str, date: Optional[datetime] ):
        item = Income(amount = amount, type = income_type, description = description, date = date)
        self._repo.add_income(item)

# доделать
    def add_obligation(self, name: str, obligation_type: ObligationType, amount: float, due_date: Optional[datetime] = None, start_date: Optional[datetime] = None, monthly_payment: float = 0.0, paid_amount: float = 0.0, description: str = "") -> None:
        if due_date is None:
            due_date = datetime.now()
        if start_date is None:
            start_date = datetime.now()
        item = Obligation(name=name, type=obligation_type, amount=amount, due_date=due_date, start_date=start_date, monthly_payment=monthly_payment, paid_amount=paid_amount, description=description,)
        self._repo.add_obligation(item)

# UPDATE

    def update_expense(self, expense: Expense) -> None:
        item = self._repo.get_expense_by_id(expense.id)
        expense.updated_at = datetime.now()
        self._repo.update_expense(expense)

    def update_income(self, income: Income) -> None:
        item = self._repo.get_income_by_id(income.id)
        income.updated_at = datetime.now()
        self._repo.update_income(income)

    def update_obligation(self, obligation: Obligation) -> None:
        item = self._repo.get_obligation_by_id(obligation.id)
        obligation.update_at = datetime.now()
